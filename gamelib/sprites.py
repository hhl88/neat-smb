...
class Player(Collidable):
    ...
    def update(self):
        ...
        if self.is_long_jump and not self.springing:
            self.jump_accel = 0.3
        elif self.is_short_jump:
            self.jump_accel = 0.6
    ...
    def move_decision(self, direction):
        if direction == "RIGHT":
            self.is_right = True
        elif direction == "LEFT":
            self.is_left = True
        elif direction == "LONG JUMP":
            self.jump()
            self.is_long_jump = True
        elif direction == "SHORT JUMP":
            self.jump()
            self.is_short_jump = True
